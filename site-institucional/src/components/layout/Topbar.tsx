...
function NotificationBell() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch(`${API_BASE}/api/notifications/logs?limit=5`, {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {count > 0 && (
          <span className="absolute top-1 right-1 w-2 h-2 bg-[var(--imc-ouro)] rounded-full" />
        )}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl border border-slate-200/60 shadow-lg z-50 overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-800">Notificacoes</p>
          </div>
          ...
          <Link href="/notificacoes" onClick={() => setOpen(false)} className="block p-3 text-center text-xs font-medium text-[var(--imc-verde-terra)] hover:bg-slate-50 transition-colors">
            Ver todas
          </Link>
        </div>
      )}
...
function MessagesIcon() {
  ...
  return (
    <Link href="/chat" className="relative p-2 rounded-xl hover:bg-slate-100 transition-colors">
      ...
      {count > 0 && (
        <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 bg-[var(--imc-verde-online)] text-white text-[9px] font-bold rounded-full flex items-center justify-center">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </Link>
...
function AvatarMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="w-[26px] h-[26px] rounded-full bg-gradient-to-br from-[var(--imc-verde-terra)] to-emerald-700 flex items-center justify-center text-[10px] font-bold text-white shadow-sm hover:shadow-md transition-shadow cursor-pointer"
      >
        {initials}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-slate-200/60 shadow-lg z-50 overflow-hidden">
          <div className="p-3 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-800 truncate">{user?.nome || "Utilizador"}</p>
          ...
          <div className="border-t border-slate-100 py-1">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors w-full text-left cursor-pointer"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
