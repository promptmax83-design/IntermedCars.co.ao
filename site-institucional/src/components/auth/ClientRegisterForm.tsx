...
export default function ClientRegisterForm({ onBack, onSuccess }: Props) {
  const [regMethod, setRegMethod] = useState<"choose" | "email" | "phone">("choose");
  ...
  const registerUser = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/register`, {
        ...
        body: JSON.stringify({
          nome: form.nome,
          email: verifyMethod === "email" ? form.email : `${form.telemovel}@intermedcars.local`,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (verifyMethod === "email" && !form.email.trim()) {
      setError("Email e obrigatorio");
      return;
    }

    if (verifyMethod === "phone" && !form.telemovel.trim()) {
      setError("Telemovel e obrigatorio");
      return;
    }
  ...
  const canSubmit =
    ...
    (verifyMethod === "email" ? form.email : form.telemovel);
    ...
    <div className="space-y-4">
      ...
      {step === "form" && regMethod === "choose" && (
        <div>
          ...
          <button
            onClick={onBack}
            className="text-[12px] text-slate-500 hover:text-slate-800 transition-colors mt-6 flex items-center gap-1"
          >
