...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060608]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {highlights.map((h) => (
              <div key={h.label} className="flex items-center gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                ...
                <p className="text-[13px] font-medium text-[#fafafa]">{h.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 lg:py-28" id="como-funciona">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-2xl lg:text-3xl font-bold text-[#fafafa] mb-4">Como Funciona</h2>
            <p className="text-[15px] text-[#71717a] max-w-md mx-auto">Do registo à entrega, acompanhamos cada etapa.</p>
          </div>
            ...
            <div className="space-y-8">
              {steps.map((s) => (
                <div key={s.num} className="flex items-start gap-6 relative">
                  ...
                  <div className="pt-2">
                    <h3 className="text-[15px] font-semibold text-[#fafafa] mb-1">{s.title}</h3>
      ...
      {/* ═══ SEGURANÇA ═══ */}
      <section className="py-20 lg:py-28 bg-[#0a0a0c]" id="seguranca">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            <p className="text-[11px] font-medium text-[#10b981] uppercase tracking-widest mb-3">Pilares</p>
            <h2 className="text-2xl lg:text-3xl font-bold text-[#fafafa] mb-4">Segurança em Primeiro Lugar</h2>
            <p className="text-[15px] text-[#71717a] max-w-md mx-auto">A confiança é a nossa maior moeda.</p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {securityPillars.map((p) => (
              <div key={p.title} className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] hover:border-[#10b981]/20 transition-colors">
                <div className="w-10 h-10 rounded-xl bg-[#10b981]/10 flex items-center justify-center mb-4">
                  <svg className="w-5 h-5 text-[#10b981]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d={p.icon} />
                  </svg>
                </div>
                <h3 className="text-[14px] font-semibold text-[#fafafa]">{p.title}</h3>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((s, i) => (
              <div key={s.label} ref={counters[i].ref} className="text-center">
                <p className="text-3xl lg:text-4xl font-bold text-[#fafafa] mb-1">
      ...
      {/* ═══ DEPOIMENTOS ═══ */}
      <section className="py-20 lg:py-28 bg-[#0a0a0c]">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            <p className="text-[11px] font-medium text-[#f59e0b] uppercase tracking-widest mb-3">Depoimentos</p>
            <h2 className="text-2xl lg:text-3xl font-bold text-[#fafafa]">O que dizem os nossos utilizadores</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {testimonials.map((t) => (
              <div key={t.name} className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.04]">
                ...
                <p className="text-[13px] text-[#a1a1aa] leading-relaxed mb-4">&ldquo;{t.text}&rdquo;</p>
                <div>
                  <p className="text-[13px] font-semibold text-[#fafafa]">{t.name}</p>
      ...
      <section className="py-20 lg:py-28">
        <div className="max-w-3xl mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-2xl lg:text-3xl font-bold text-[#fafafa]">Perguntas Frequentes</h2>
          </div>
          ...
          <div className="space-y-3">
            {faqs.map((f, i) => (
              <div key={i} className="rounded-xl bg-white/[0.02] border border-white/[0.04] overflow-hidden">
                ...
                >
                  <span className="text-[14px] font-medium text-[#fafafa] pr-4">{f.q}</span>
                  <svg className={`w-5 h-5 text-[#71717a] flex-shrink-0 transition-transform ${openFaq === i ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                  </svg>
                </button>
                {openFaq === i && (
                  <div className="px-5 pb-5">
                    <p className="text-[13px] text-[#71717a] leading-relaxed">{f.a}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      <section className="py-20 lg:py-28">
        <div className="max-w-3xl mx-auto px-4 lg:px-8 text-center">
          <div className="p-8 lg:p-12 rounded-3xl bg-gradient-to-br from-[#10b981]/10 via-[#060608] to-[#f59e0b]/5 border border-white/[0.04]">
            <h2 className="text-2xl lg:text-3xl font-bold text-[#fafafa] mb-4">
              Pronto para começar?
            </h2>
            <p className="text-[15px] text-[#71717a] mb-8 max-w-md mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href="/explorar" className="px-8 py-3 bg-white/[0.04] border border-white/[0.06] hover:border-[#f59e0b]/40 text-[#fafafa] text-sm font-semibold rounded-xl transition-all">
                Explorar Veículos
              </Link>
            </div>
          </div>
