...
class ContractService
{
    ...
    {
        $sql = 'SELECT n.*, v.marca, v.modelo, v.ano, v.preco as vehicle_price, v.combustivel, v.caixa, v.cor, v.km,
    ...
    {
        $preco = number_format((float) $data['final_car_price_aoa'], 2, ',', '.');
        $comissao = number_format((float) $data['final_car_price_aoa'] * 0.02, 2, ',', '.');
        $date = date('d/m/Y');
        $hora = date('H:i');
        $iban = $_ENV['PAYMENT_IBAN'] ?? getenv('PAYMENT_IBAN') ?: 'N/D';
        $beneficiary = $_ENV['PAYMENT_BENEFICIARY'] ?? getenv('PAYMENT_BENEFICIARY') ?: 'N/D';
        $buyerName = $data['buyer_name'] ?? 'N/D';
        $buyerEmail = $data['buyer_email'] ?? 'N/D';
        $buyerPhone = $data['buyer_phone'] ?? 'N/D';
        $buyerBi = $data['buyer_bi'] ?? 'N/D';
...
<body>
    ...
    <div class="section">
        ...
        <div class="field"><span class="label">Comprador:</span><span class="value">{$buyerName}</span></div>
        <div class="field"><span class="label">BI/Passaporte:</span><span class="value">{$buyerBi}</span></div>
        <div class="field"><span class="label">Email:</span><span class="value">{$buyerEmail}</span></div>
        <div class="field"><span class="label">Telemovel:</span><span class="value">{$buyerPhone}</span></div>
    </div>
