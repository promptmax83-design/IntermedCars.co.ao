...
class TransactionService
{
    ...
    public function createProposal(int $vehicleId, int $buyerId, int $sellerId, float $proposedPrice): array
    {
        ...
        $sql = 'INSERT INTO transactions (vehicle_id, buyer_id, seller_id, proposed_price, status, created_at)
                VALUES (:vehicle_id, :buyer_id, :seller_id, :proposed_price, :status, CURRENT_TIMESTAMP)';
    ...
    {
        $sql = 'UPDATE transactions SET status = :status, accepted_at = CURRENT_TIMESTAMP WHERE id = :id AND seller_id = :seller_id';
        $stmt = Database::getConnection()->prepare($sql);
        $stmt->execute([
    ...
    public function recordDeposit(int $transactionId, int $buyerId, float $amount): array
    {
        ...
        $sql = 'UPDATE transactions SET status = :status, deposit_amount = :amount, deposited_at = CURRENT_TIMESTAMP
    ...
    public function completeInspection(int $transactionId, bool $approved): array
    {
        ...
        $sql = 'UPDATE transactions SET status = :status, inspection_completed_at = CURRENT_TIMESTAMP,
    ...
    {
        // Record the payment
        // TODO: Implementar método recordPayment() no CommissionService ou criar repositório fee_payments
        ...
        if ($bothPaid) {
            $sql = 'UPDATE transactions SET status = :status, completed_at = CURRENT_TIMESTAMP WHERE id = :id';
            $stmt = Database::getConnection()->prepare($sql);
            $stmt->execute([
