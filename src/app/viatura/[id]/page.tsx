...
const viatura = {
  ...
  descricao:
    "BMW Serie 5 530e M Sport em excelente estado. Equipada com pacote M Sport, navigador profissional, tetosolar panorâmico, câmaras 360° e assistente de estacionamento. Revisões em dia na marca.",
};
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors ${favoritado ? "bg-red-50 text-red-500" : "bg-gray-100 text-gray-400"}`}
          >
            {favoritado ? "♥" : "♡"}
          </button>
          <button className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 hover:text-foreground transition-colors">
            ↗
          </button>
          <button className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 hover:text-foreground transition-colors">
            ⚖
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
            {galeria.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-colors ${i === imagemAtual ? "bg-accent" : "bg-gray-300"}`}
              />
            ))}
          </div>
        ...
        <div className="flex gap-1 p-2 overflow-x-auto scrollbar-hide">
          {galeria.map((img, i) => (
            <button
              key={i}
              onClick={() => setImagemAtual(i)}
              className={`w-16 h-16 rounded-lg flex items-center justify-center shrink-0 border-2 transition-colors ${i === imagemAtual ? "border-accent" : "border-transparent"}`}
            >
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-muted text-sm mt-1">
              📍 {viatura.local} · 🕐 Publicado há 3 dias
            </p>
          </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {proprietario.verificado && (
                <span className="text-accent text-sm">✓ Verificado</span>
              )}
            </div>
            <p className="text-xs text-muted">
              {proprietario.tipo} · ★ {proprietario.rating} (
              {proprietario.avaliacoes} avaliações) · {proprietario.vendas}{" "}
              vendas
            </p>
          </div>
          <Link
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-accent rounded-full flex items-center justify-center text-primary font-bold">
            AR
          </div>
          <div className="flex-1">
            <p className="font-bold">Ana Rodrigues</p>
            <p className="text-xs text-muted">
              Agente Certificado · ★ 4.9 · 23 vendas este mês
            </p>
          </div>
          <Link href="/agente/ana" className="text-sm text-accent font-medium">
            Ver perfil →
          </Link>
        </div>
      </div>
      ...
      <div className="sticky bottom-4 flex gap-3">
        <Link
          href="/financiamento"
          className="flex-1 bg-white border border-gray-200 text-center py-3 rounded-xl font-medium text-sm hover:bg-light transition-colors"
        >
          💳 Financiar
        </Link>
        <Link
          href="/chat"
          className="flex-1 bg-accent text-primary text-center py-3 rounded-xl font-bold text-sm hover:bg-accent-hover transition-colors"
        >
          💬 Iniciar Negociação
        </Link>
      </div>
