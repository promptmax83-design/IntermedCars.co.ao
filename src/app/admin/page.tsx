...
const stats = [
  { valor: "1.247", label: "Utilizadores", change: "+12%", cor: "text-blue-500" },
  { valor: "89", label: "Agentes", change: "+5", cor: "text-accent" },
  { valor: "34", label: "Stands", change: "+2", cor: "text-emerald-500" },
  { valor: "156", label: "Mediações Ativas", change: "+18", cor: "text-amber-500" },
  { valor: "€2.4M", label: "Volume Total", change: "+23%", cor: "text-accent" },
  { valor: "€180k", label: "Comissões", change: "+15%", cor: "text-emerald-500" },
];

const tabs = ["Dashboard", "Utilizadores", "Agentes", "Stands", "Mediações", "Veículos", "Pagamentos", "Escrow", "Denúncias", "Suporte", "Analytics", "Logs", "Permissões"];
...
const utilizadores = [
  ...
  { nome: "Pedro Santos", tipo: "Comprador", status: "Pendente", data: "07/2026" },
    ...
    <div className="flex h-full overflow-hidden">
      <div className="hidden lg:flex flex-col w-52 bg-white border-r border-gray-200 shrink-0 py-4 px-3">
        ...
        <div className="space-y-0.5">
          {tabs.map((t) => (
            <button key={t} onClick={() => setTabAtiva(t)} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-colors ${t === tabAtiva ? "bg-primary text-white" : "text-gray-600 hover:bg-light"}`}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="flex items-center gap-2 lg:hidden mb-4 overflow-x-auto scrollbar-hide">
          {tabs.map((t) => (
            <button key={t} onClick={() => setTabAtiva(t)} className={`px-3 py-1.5 rounded-full text-xs font-medium shrink-0 ${t === tabAtiva ? "bg-primary text-white" : "bg-white border border-gray-200"}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tabAtiva === "Utilizadores" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <input type="text" placeholder="Pesquisar..." className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" />
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-light">
                  <tr><th className="text-left px-4 py-3 font-medium">Nome</th><th className="text-left px-4 py-3 font-medium">Tipo</th><th className="text-left px-4 py-3 font-medium">Estado</th><th className="text-left px-4 py-3 font-medium">Desde</th></tr>
                </thead>
                <tbody>
                  {utilizadores.map((u, i) => (
                    <tr key={i} className="border-t border-gray-100">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${u.status === "Ativo" ? "bg-emerald-50 text-emerald-600" : u.status === "Pendente" ? "bg-amber-50 text-amber-600" : "bg-red-50 text-red-600"}`}>
                          {u.status}
                        </span>
                      </td>
