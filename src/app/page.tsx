...
const viaturas = [
  { id: 1, marca: "BMW", modelo: "Serie 5 530e", ano: "2023", preco: "42.900", km: "15.000", combustivel: "Híbrido", local: "Lisboa", vendedor: "Stand AutoPremium", patrocinado: true, imagem: "🚗" },
  { id: 2, marca: "Porsche", modelo: "Macan S", ano: "2022", preco: "68.900", km: "18.000", combustivel: "Gasolina", local: "Porto", vendedor: "Carlos M.", patrocinado: false, imagem: "🏎️" },
  { id: 3, marca: "Tesla", modelo: "Model 3 LR", ano: "2023", preco: "44.900", km: "12.000", combustivel: "Elétrico", local: "Braga", vendedor: "EcoCars", patrocinado: false, imagem: "⚡" },
  { id: 4, marca: "Mercedes-Benz", modelo: "Classe C 200", ano: "2022", preco: "38.500", km: "22.000", combustivel: "Gasolina", local: "Lisboa", vendedor: "Premium Motors", patrocinado: true, imagem: "🚗" },
  { id: 5, marca: "Audi", modelo: "Q5 40 TDI", ano: "2023", preco: "49.900", km: "10.000", combustivel: "Diesel", local: "Faro", vendedor: "Ana S.", patrocinado: false, imagem: "🚙" },
  { id: 6, marca: "Volvo", modelo: "XC60 B5", ano: "2023", preco: "52.900", km: "8.000", combustivel: "Híbrido", local: "Coimbra", vendedor: "NordAuto", patrocinado: false, imagem: "🚙" },
];

const nearby = [
  { marca: "VW", modelo: "Golf 8 GTI", preco: "33.900", distancia: "2.3 km" },
  { marca: "Hyundai", modelo: "Tucson N-Line", preco: "35.900", distancia: "4.1 km" },
  { marca: "Toyota", modelo: "RAV4 Hybrid", preco: "36.500", distancia: "5.8 km" },
];
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {stories.map((s) => (
              <div key={s.nome} className="flex flex-col items-center gap-1 shrink-0">
                <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${s.cor} flex items-center justify-center border-2 border-accent`}>
                  <span className="text-white text-xs font-bold">{s.nome.slice(0, 2)}</span>
                </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-semibold shrink-0 transition-colors ${
                selectedCategoria === "Todas" ? "bg-primary text-white" : "bg-white text-gray-600 border border-gray-200"
              }`}
            >
              Todas
            </button>
            {categorias.map((c) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold shrink-0 transition-colors flex items-center gap-1 ${
                  selectedCategoria === c.nome ? "bg-primary text-white" : "bg-white text-gray-600 border border-gray-200"
                }`}
              >
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/explorar" className="text-accent text-xs font-medium">Ver todos</Link>
            </div>
            <div className="flex gap-3 overflow-x-auto scrollbar-hide">
              {nearby.map((n) => (
                <div key={`${n.marca}-${n.modelo}`} className="bg-light rounded-lg p-3 min-w-[140px] shrink-0">
                  <p className="text-[10px] text-muted">{n.distancia}</p>
                  <p className="text-xs font-bold">{n.marca} {n.modelo}</p>
                  <p className="text-accent text-sm font-bold mt-1">{n.preco}€</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted">{viaturas.length} viaturas</span>
            </div>
            {viaturas.map((car) => (
              <Link
                key={car.id}
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <p className="text-accent font-bold text-lg">{car.preco}€</p>
                  </div>
      ...
      <div className="hidden xl:block w-80 border-l border-gray-200 bg-white overflow-y-auto shrink-0">
        ...
        <div className="p-4 space-y-3">
          {viaturas.slice(0, 3).map((car) => (
            <Link key={car.id} href={`/viatura/${car.id}`} className="flex gap-3 p-2 rounded-lg hover:bg-light transition-colors">
              <div className="w-16 h-16 bg-light rounded-lg flex items-center justify-center text-2xl shrink-0">{car.imagem}</div>
        ...
        <div className="p-4 border-t border-gray-100">
          ...
          {[
            ...
            <Link key={agente.nome} href={`/agente/${agente.nome}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-light transition-colors">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted">{agente.vendas} vendas · ★ {agente.rating}</p>
              </div>
            </Link>
          ))}
        </div>
