...
class AuthController extends BaseController
{
    ...
    public function register(array $data): array
    {
        ...
        $regMethod = $data['reg_method'] ?? 'email';
        if ($regMethod === 'phone') {
            if (empty($data['telemovel'])) {
                throw new \InvalidArgumentException("Telemovel obrigatorio para registo por telemovel");
            }
            if (!preg_match('/^(\+244)?9\d{8}$/', $data['telemovel'])) {
                throw new \InvalidArgumentException("Telemovel invalido");
            }
        } else {
            // Email-only registration: telemovel is optional
            if (!empty($data['telemovel']) && !preg_match('/^(\+244)?9\d{8}$/', $data['telemovel'])) {
                throw new \InvalidArgumentException("Telemovel invalido");
            }
        }
        ...
        $verificationService = new \IntermedCars\Services\VerificationService();
        if (!$verificationService->isEmailVerified($data['email'])) {
            throw new \InvalidArgumentException("Email nao verificado. Confirme o codigo enviado.");
        }
        if ($regMethod === 'phone' && !$verificationService->isPhoneVerified($data['telemovel'] ?? '')) {
            throw new \InvalidArgumentException("Telemovel nao verificado. Confirme o codigo enviado.");
        }
        ...
        $sql = 'INSERT INTO users (nome, email, telemovel, bi_passaporte, password_hash, nome_pai, nome_mae, status, created_at)
                VALUES (:nome, :email, :telemovel, :bi_passaporte, :password_hash, :nome_pai, :nome_mae, :status, CURRENT_TIMESTAMP)';
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            'nome' => $data['nome'],
            'email' => $data['email'],
            'telemovel' => $data['telemovel'] ?? '000000000',
    ...
    public function uploadBiFrente(int $userId, string $filePath): array
    {
        ...
        $sql = 'UPDATE users SET bi_frente_path = :path, updated_at = CURRENT_TIMESTAMP WHERE id = :id';
        $stmt = $this->db->prepare($sql);
    ...
    public function uploadBiVerso(int $userId, string $filePath): array
    {
        ...
        $sql = 'UPDATE users SET bi_verso_path = :path, updated_at = CURRENT_TIMESTAMP WHERE id = :id';
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['path' => $encryptedPath, 'id' => $userId]);
    ...
    public function uploadSelfie(int $userId, string $filePath): array
    {
        ...
        $sql = 'UPDATE users SET selfie_path = :path, updated_at = CURRENT_TIMESTAMP WHERE id = :id';
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['path' => $encryptedPath, 'id' => $userId]);
    ...
    public function processKyc(int $userId): array
    {
        ...
        if ($verified) {
            $sql = 'UPDATE users SET status = :status, verified_at = CURRENT_TIMESTAMP WHERE id = :id';
            $stmt = $this->db->prepare($sql);
